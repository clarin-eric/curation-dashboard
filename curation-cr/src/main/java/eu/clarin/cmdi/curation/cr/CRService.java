...
public class CRService implements ICRService {
   ...
   public static final String CR_REST = vloConf.getComponentRegistryRESTURL()
         .replaceFirst("http(s)?", "(http|https)").replaceFirst("/1\\..+", "/.+");
   ...
   private static final LoadingCache<ProfileHeader, ProfileCacheEntry> publicProfilesCache = ProfileCacheFactory
         .createProfileCache(true);
   private static final LoadingCache<ProfileHeader, ProfileCacheEntry> nonpublicProfilesCache = ProfileCacheFactory
         .createProfileCache(false);

   // cache for score
   private static final LoadingCache<ProfileHeader, Double> publicScoreCache = ProfileScoreCacheFactory
         .createScoreCache(true);
   private static final LoadingCache<ProfileHeader, Double> nonpublicScoreCache = ProfileScoreCacheFactory
         .createScoreCache(false);
   ...
   public ProfileHeader createProfileHeader(String schemaLocation, String cmdiVersion, boolean isLocalFile) {
      ...
      if (!isLocalFile && schemaLocation.startsWith(vloConf.getComponentRegistryRESTURL()))
