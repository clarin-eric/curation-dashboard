 ...
 *
 */
public class MappingCreatorThread extends Thread {

    static final Logger _logger = LoggerFactory.getLogger(MappingCreatorThread.class);

    private String profile;
    private String xsdFile;
    private FacetConceptMapping conceptMapping;

    public MappingCreatorThread(String profile, FacetConceptMapping conceptMapping) throws Exception {
	this.profile = profile;
	this.conceptMapping = conceptMapping;
	this.xsdFile = ComponentRegistryService.getInstance().getSchemaFile(profile).getAbsolutePath();
    }
    
    public void run(){
	Profile2FacetMap mapping = createMapping();
    }

    public Profile2FacetMap createMapping() {
	Profile2FacetMap result = new Profile2FacetMap();
	try {
	    // The magic
	    ...
	    for (FacetConcept facetConcept : conceptMapping.facetConcepts) {
		...
		for (String concept : facetConcept.getConcepts()) {
			...
			if (facetConcept.hasContext()) {
				...
				// lazily instantiate the reverse mapping, i.e.,
		...
		// pattern-based blacklisting: remove all XPath expressions that
		...
		if (!config.getPatterns().isEmpty() || !config.getFallbackPatterns().isEmpty()) {
		    result.addMapping(config);
		}
	    }
	} catch (NavException e) {
	    _logger.error("Error creating facetMapping from xsd: {}", profile, e);
	}
	return result;
    }
     ...
     * @throws NavException
     */
    private Map<String, List<String>> createConceptLinkPathMapping() throws NavException {
        Map<String, List<String>> result = new HashMap<String, List<String>>();
        VTDGen vg = new VTDGen();
        boolean parseSuccess;
        parseSuccess = vg.parseFile(xsdFile, true);
      
        if (!parseSuccess) {
        	_logger.error("Cannot create ConceptLink Map from xsd (xsd is probably not reachable): " + profile + ". All metadata instances that use this xsd will not be imported correctly.");
