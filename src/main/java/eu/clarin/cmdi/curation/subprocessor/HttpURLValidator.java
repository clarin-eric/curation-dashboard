...
public class HttpURLValidator extends CurationTask<CMDIRecord> {
	...
	// filter urls
//	Collection<CMDIUrlNode> urls = entity.getValues().stream()
//		.filter(value -> value.getValue().startsWith("http://") || value.getValue().startsWith("https://"))
//		.collect(Collectors.toList());

	Collection<CMDIUrlNode> urls = entity.getValues();
	...
	urls.parallelStream().forEach(value -> {
		...
		if (responseCode == 200 || responseCode == 302) {
		    report.addMessage(new Message(Severity.DEBUG, tag + "URL: " + link + "\t STATUS:" + responseCode));
					    ...
					    // content ...
		    report.addMessage(
			    new Message(Severity.WARNING, tag + "URL: " + link + "\t STATUS:" + responseCode));
	...
	report.addMessage(
		"Number of broken links: " + numOfBrokenLinks + "\t" + percent(numOfBrokenLinks.get(), urls.size()));
	return report;
